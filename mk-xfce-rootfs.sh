...
if [ ! $TARGET ]; then
	TARGET='xfce'
    echo -e "\033[47;36m set default TARGET=xfce...... \033[0m"
fi
...
if [ $MIRROR ]; then
	...
	echo "deb [arch=arm64 signed-by=/etc/apt/keyrings/embedfire.gpg] https://cloud.embedfire.com/mirrors/ebf-debian $MIRROR main" | tee /etc/apt/sources.list.d/embedfire.list > /dev/null
fi
...
ln -sf /lib/aarch64-linux-gnu/libstdc++.so.6 /usr/lib/chromium/libstdc++.so.6
